...
$stmt = $conn->prepare('SELECT order_status, IFNULL(delivery_status, order_status) AS EffectiveDelivery FROM orders WHERE order_id=? AND customer_id=? LIMIT 1');
...
$sqlUp = $hasCompletedAt ? "UPDATE orders SET order_status='Completed', delivery_status='Completed', CompletedAt=NOW() WHERE order_id=?" : "UPDATE orders SET order_status='Completed', delivery_status='Completed' WHERE order_id=?";
...
// Return aliased keys to match client expectations
$stmt2 = $conn->prepare('SELECT order_status AS OrderStatus, delivery_status AS DeliveryStatus FROM orders WHERE order_id=? LIMIT 1');
if($stmt2){
