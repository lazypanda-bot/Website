// Quick Order Modal accessibility & body scroll lock helpers
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const buyNowBtns = Array.from(document.querySelectorAll('.buy-btn'));
    const addToCartBtn = document.querySelector('.addcart-btn');
    const cartIcon = document.querySelector('.cart-icon i');
    // keep last prepared product in module scope for quick-order confirm flows
    let pendingProduct = null;

    function getSelectedProduct() {
        // Get product name. Prefer an input with id 'product-name' when present, otherwise fall back to the visible product title.
        let name = (document.getElementById('product-name') && document.getElementById('product-name').value) || 
    ...
    if (buyNowBtns.length) {
        // mark that an external buy-now handler is attached so fallbacks can skip
        window.__buyNowHandled = true;
    ...
    buyNowBtns.forEach(buyNowBtn => buyNowBtn.addEventListener('click', async (e) => {
            ...
            // (avoids storing JSON on DOM elements which can lead to conflicts)
            // pendingProduct already set above.
            // Add to cart (attempt server add when possible, fallback to local storage)
            try {
                await addToCart(product, { serverPreferred: true });
            } catch (err) {
                // In case of unexpected error, fallback to local add
        ...
        quickOrderConfirmBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                try { 
                    ...
                    console.warn('Non-JSON response:', dataText); data = { status:'error', message:'Invalid server response', raw:dataText }; 
                }
