...
CREATE TABLE `orders` (
  ...
  `OrderStatus` ENUM('Pending','Processing','Ready','Shipped','Completed','Cancelled','Ready for Pickup','Ready to Ship') NOT NULL DEFAULT 'Pending',
  `DeliveryAddress` VARCHAR(255) NULL,
  `DeliveryStatus` ENUM('Pending','Shipped','Delivered','Completed','Picked up','Failed') NOT NULL DEFAULT 'Pending',
  `phone_number` VARCHAR(30) NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
